package br.com.fiap.tds.tdspa.javaadv.blogBackend.infrastructure.runner;
import br.com.fiap.tds.tdspa.javaadv.blogBackend.datasource.repositories.*;
...
//            user2.setPassword(passwordEncoder.encode("masterkey"));
            ...
            IntStream.rangeClosed(1, 1000).forEach(i -> {
                ...
                OrderKey orderKey = new OrderKey(user.getId(), UUID.randomUUID());
                Order order = new Order();
                order.setKey(orderKey);

                order.setTotalQuantity(random.nextInt(10) + 1);
                order.setTotalPrice(1000 + random.nextDouble() * 950); // entre 50 e 1000
                order.setUser(user);
